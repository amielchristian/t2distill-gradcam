...
def load_weights_numpy(
    ...
    for match in matches:
        ...
        source_weights = [params_dict[str.replace(k, '-', '/')] for k in match["keys"]]
    ...
    unused = set(source_keys).difference(str.replace(k, '-', '/') for k in source_keys_used) # just for peace of mind
    if unused:
